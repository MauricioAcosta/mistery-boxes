 ...
 *   reeling   → Horizontal slot-machine reel scrolls to the winner (5 200 ms).
 *   revealing → Winner card zooms in with star-burst particles (1 000 ms).
 *
 * Props:
...
const REEL_SIZE = 60    // total cards in reel
const WINNER_IDX = 44   // winner position
const REEL_MS   = 5200  // reel animation duration

/* ── Phase timings ───────────────────────────────────────────── */
const SHAKE_MS = 700
const BURST_MS = 500

const P = { IDLE: 'idle', SHAKING: 'shaking', BURSTING: 'bursting', REELING: 'reeling', REVEALING: 'revealing' }
...
export default function BoxOpenScene({ box, animData, onFinish }) {
  ...
  useEffect(() => {
    ...
    const t4 = setTimeout(() => { onFinish?.() },       total + 1000)

    return () => [t1, t2, t3, t4].forEach(clearTimeout)
  }, [animData])   // eslint-disable-line react-hooks/exhaustive-deps

  /* Scroll reel to winner when reeling phase starts ───────────── */
  useEffect(() => {
    ...
    const jitter      = (Math.random() - 0.5) * 60     // natural imprecision
    const targetX     = -(WINNER_IDX * CARD_W - centerOff + jitter)

    requestAnimationFrame(() => {
      el.style.transition = `transform ${REEL_MS}ms cubic-bezier(0.12, 0.9, 0.2, 1)`
      ...
      <div className="scene-stage">
        <div className="scene-reveal-wrap">
          ...
          >
            ...
            <span className="scene-winner-value">
